...
for f in range(len(file_list)):
    ...
    df_weekly = df_daily.resample("W", on="Date").mean()
    ...
    file_daily = 'daily' + file_list[f]
    file_weekly = 'weekly' + file_list[f]
    
    export_daily = df_daily.to_csv(PATH_2 + 'Daily' + '/' + 'daily_' + file_list[f], 
                                   sep=';', index=False, header=True, encoding='utf-8')
    
    export_weekly = df_weekly.to_csv(PATH_2 + 'Weekly' + '/' + 'weekly' + file_list[f], 
                                   sep=';', index=False, header=True, encoding='utf-8')
